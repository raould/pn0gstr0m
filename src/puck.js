...
function Puck() {
    ...
    self.MaybeSplitPuck = function({ forced=false, isSuddenDeath=false, maxVX }) {
    ...
    self.ApplyEnglish = function( paddle ) { // note: pretty minimal, on purpose.
    ...
    self.PaddleCollisionNewprops = function( paddle, isSuddenDeath, maxVX ) {
        ...
        if ( hit ) {
            ...
            if (paddle.isSplitter) {
                newprops = self.MaybeSplitPuck({ isSuddenDeath, maxVX });
            }
        }
        return newprops;
    };
