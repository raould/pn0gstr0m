...
function is1P() {
  // fails to account for attract mode.
...
function ForGameMode(_ref) {
  ...
  if (gGameMode === kGameModeRegular) {
    ...
    return exists(pp) ? pp : exists(zen) ? zen : exists(hard) ? hard : regular;
  }
}
...
function ForKeyMode(mode, down, up) {
  if (mode === kKeyDown) {
    return down;
  } else {
    return up;
...
var gPuckLeftCount = 0;
var gPuckRightCount = 0;
var gPuckYMin = gHeight;
var gPuckYMax = 0;
var gPuckYAvg = gh(0.5);
var kPuckYCountBuckets = 10;
var kPuckYCountBucketHeight = gHeight / kPuckYCountBuckets;
var gPuckYLeftCounts = new Array(kPuckYCountBuckets).fill(0);
var gPuckYRightCounts = new Array(kPuckYCountBuckets).fill(0);
var gPuckYLeftCommonIndex = undefined;
var gPuckYRightCommonIndex = undefined;
    ...
    Cxdo(function () {
      ...
      ForGameMode({
        regular: function regular() {
          ...
          DrawText("LEVEL ".concat(gLevelInt), "center", gw(0.5), gh(0.08), gSmallestFontSizePt);
        },
...
function GameState(props) {
  ...
  self.SetPaused = function () {
    ...
    if (self.paused != pause) {
      ...
      if (self.paused && exists(self.theMenu)) {
        ...
        var _self$theMenu3;
        if ((_self$theMenu3 = self.theMenu) != null && _self$theMenu3.isOpen()) {
          var _self$theMenu4;
          (_self$theMenu4 = self.theMenu) == null || _self$theMenu4.bMenu.Click(); // sure hope this stays in sync.
          clearAnyMenuPressed(); // todo: code smell.
        }
      }
    }
  ...
  self.Step = function (dt) {
    var _self$theMenu5;
    (_self$theMenu5 = self.theMenu) == null || _self$theMenu5.Step(); // fyi this doesn't process menu inputs, that is below.
    self.level.Step(dt);
    self.maxVX = self.level.maxVX; // todo: code smell global.
    self.MaybeSpawnPills(dt);
    self.StepDarkMatter(dt);
    if (self.hintTimeout > 0) {
      self.hintTimeout -= dt;
    }
    self.ProcessAllInput();
    if (self.quit != false) {
      // could be false, true, "won", or "lost". :-(
  ...
  self.CreateRandomPucks = function (props) {
    var count = props.count,
      ry = props.y,
      vy = props.vy;
    var y = ry != null ? ry : gh(gR.RandomRange(0.1, 0.9));
    for (var i = 0; i < count; ++i) {
      var p = gPuckPool.Alloc();
      if (exists(p)) {
        p.PlacementInit({
          ...
          vy: vy != null ? vy : gR.RandomCentered(0, 0.15, 0.01),
          ur: true
        });
        gPucks.A.push(p);
      }
  ...
  self.ProcessOneInput = function (cmds) {
    ...
    if ((_self$theMenu6 = self.theMenu) != null && _self$theMenu6.ProcessOneInput(cmds)) {
      return;
    }
    var pbp = false;
  ...
  self.Draw = function (props) {
    ...
    if (!gResizing) {
      ...
      (_self$darkMatter4 = self.darkMatter) == null || _self$darkMatter4.Draw(self.Alpha());
      if (!isEndScreenshot) {
        var _self$theMenu7;
        self.DrawHint();
        gSparks.A.forEach(function (s) {
        ...
        self.DrawAnimations();
        (_self$theMenu7 = self.theMenu) == null || _self$theMenu7.Draw();
  ...
  self.DrawDebug = function () {
    ...
    Cxdo(function () {
      ...
      gCx.fillStyle = "grey";
      DrawText("YMIN:".concat(ii(gPuckYMin)), "left", gw(0.8), gh(0.8), gSmallestFontSizePt);
      DrawText("YMAX:".concat(ii(gPuckYMax)), "left", gw(0.8), gh(0.85), gSmallestFontSizePt);
