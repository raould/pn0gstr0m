...
function Paddle(props) {
  ...
  */
  ...
  self.AddYars = function (props) {
    self.yars = new Blocks(_objectSpread(_objectSpread({}, props), {}, {
      isYars: true
    }));
  };
  ...
  self.Draw = function (alpha, gameState, s01, isEndScreenshot) {
    ...
    (_self$yars = self.yars) == null || _self$yars.Draw(alpha);
    if (exists(self.hp)) {
    ...
    if (gDebug) {
      Cxdo(function () {
        if (false === self.isPlayer && false === self.isXtra) {
          gCx.fillStyle = "grey";
          var x = ForSide(self.side, gw(0.1), gw(0.9));
          var align = ForSide(self.side, "right", "left");
  ...
  self.IsUp = function () {
    return self.keyStates.some(function (ks) {
      return ks.$.up;
    }) || self.stickStates.reduce(function (up, state) {
      return up || state.$.up;
    }, false) || self.buttonStates.reduce(function (up, state) {
      return up || state.$.up;
    }, false);
  };
  ...
  self.StepAI = function (dt, gameState) {
    var _self$aiPuck, _self$aiPill;
    Assert(!self.isPlayer);
    if (false === ((_self$aiPuck = self.aiPuck) == null ? void 0 : _self$aiPuck.alive)) {
      self.aiPuck = undefined;
    }
    if (false === ((_self$aiPill = self.aiPill) == null ? void 0 : _self$aiPill.alive)) {
      self.aiPill = undefined;
    }
    self.AISeek(dt);
  ...
  self.AISeek = function (dt) {
    ...
    if (count > 10 && PS && exists(self.aiPill) && self.aiPill.isUrgent) {
      ...
      var dyf = Math.abs(self.aiPill.y - self.y) / gHeight;
      if (dyf < 0.2) {
        self.AISeekTargetMidY(dt, self.aiPill.y + self.aiPill.height / 2);
        return;
      }
    }
    var attacking_count = ForSide(self.side, gPuckLeftCount, gPuckRightCount);
    if (count < 10 || attacking_count / count > 0.25) {
      ...
      var index = ForSide(self.side, gPuckYLeftCommonIndex, gPuckYRightCommonIndex);
      if (exists(index)) {
        self.debugMsg = "PUCKS!";
        var midY = kPuckYCountBucketHeight * index - kPuckYCountBucketHeight / 2;
  ...
  self.shouldUpdate = function () {
    var _ref, _self$aiPuck2, _ref2, _self$aiPill2;
    self.aiCountdownToUpdate--;
    var hasPuck = (_ref = ((_self$aiPuck2 = self.aiPuck) == null ? void 0 : _self$aiPuck2.alive) === true) != null ? _ref : false;
    var hasPill = (_ref2 = ((_self$aiPill2 = self.aiPill) == null ? void 0 : _self$aiPill2.alive) === true) != null ? _ref2 : false;
